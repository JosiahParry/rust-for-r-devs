...
- A "tuple struct" with the structure `struct NewType(OtherType)`
- Helps us add our flair (aka. methods) to these structs
...
- Structs and enums owned by other crates cannot have an `impl` added to them
- That is called the **"orphan rule"**
- We can side step this with a newtype struct
...
- Register the `impl` with `extendr_module! { impl RShape; }`
