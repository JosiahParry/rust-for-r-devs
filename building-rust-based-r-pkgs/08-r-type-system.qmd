...
- To access the inner type—e.g. `f64` or `i32` we can use the `.0()` method.
...
Write a vectorized function `powi()` that raises each element of a numeric
vector to an integer power.

- It takes a `Doubles` (`x`) and an `i32` (`power`)
- It returns a `Doubles`
- Iterate over the scalar `Rfloat` elements, reach the inner `f64` with `.0`,
  and raise it with `f64::powi()`
- Collect into a `Doubles`
